...
class NotificationService {
  ...
  Future<void> _loadSettings() async {
    ...
    _lowAttendanceWarningsEnabled =
        prefs.getBool(_lowAttendanceWarningsKey) ?? true;
    _reminderMinutesBefore = prefs.getInt(_reminderMinutesKey) ?? 5;

    final summaryTimeHour = prefs.getInt('${_dailySummaryTimeKey}_hour') ?? 7;
    final summaryTimeMinute =
        prefs.getInt('${_dailySummaryTimeKey}_minute') ?? 0;
    _dailySummaryTime =
        TimeOfDay(hour: summaryTimeHour, minute: summaryTimeMinute);
  }
  ...
  Future<void> _saveSettings() async {
    ...
    await prefs.setBool(
        _lowAttendanceWarningsKey, _lowAttendanceWarningsEnabled);
    ...
    await prefs.setInt(
        '${_dailySummaryTimeKey}_minute', _dailySummaryTime.minute);
  }
  ...
  Future<void> schedulePreClassReminder(
      ClassInstance classInstance, Course course) async {
    ...
    final reminderTime =
        classDateTime.subtract(Duration(minutes: _reminderMinutesBefore));
  ...
  Future<void> schedulePostClassPrompt(
      ClassInstance classInstance, Course course) async {
  ...
  Future<void> scheduleDailySummary(
      List<ClassInstance> todaysClasses, List<Course> courses) async {
  ...
  Future<void> showLowAttendanceWarning(
      Course course, double currentPercentage) async {
    ...
    await _notifications.show(
      ...
      const NotificationDetails(
        android: AndroidNotificationDetails(
          ...
          channelDescription:
              'Warnings when your attendance drops below required',
          importance: Importance.high,
          priority: Priority.high,
          icon: '@mipmap/ic_launcher',
        ),
  ...
  int _calculateClassesToRecover(
      int totalClasses, int attendedClasses, double requiredPercentage) {
  ...
  Future<void> scheduleNotificationsForClass(
      ClassInstance classInstance, Course course) async {
